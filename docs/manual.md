...
PLATEAU QGISプラグイン（以下「本プラグイン」という）は、QGISでPLATEAUのCityGMLファイルを読み込み、QGIS上に表示するためのプラグインです。データは一時スクラッチレイヤとして読み込まれます。
### 対応データ形式
...
本プラグインは、QGISの公式プラグインリポジトリで公開されており、以下の手順でダウンロードが可能です。
...
6. メニュータブの[プラグイン]および、[プラグインツールバー]にインストールされたプラグインが表示されます。
