...
解凍後のフォルダ構成は変更しないようにしてください。変更すると、適切に属性が読み込めないようになります。
...
2. 入力メッシュに[TINRelief（プラグインでインポートしたDEMを選択）]を設定
3. [データセットグループ]には、右の[…]より[現在アクディブなデータセットグループ]を設定
4. [ピクセルサイズ]には、ラスタのピクセルサイズを設定
> 地形モデルは緯度経度で作成されているため、約10m（0.4秒）のラスタを作成する場合は[0.00011111]を入力します（1 / 60 / 60 * 0.4 = 0.00011111度）。
5. [出力ラスタ]には、オプションで出力先を指定
6.  ｢実行｣を押下して処理を実行します。
<img src="picture/operation23.png" width="500">

7. ラスタライズされた地形モデルが出力されます。
